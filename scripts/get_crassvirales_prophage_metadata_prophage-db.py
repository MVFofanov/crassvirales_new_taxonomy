...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input_excel", type=Path, required=True,
        help="Path to the input Excel file (.xlsx)"
    )
    parser.add_argument(
        "--output_tsv", type=Path, required=False,
        help="Path to output .tsv file (default: input name + _crassvirales.tsv)"
    )
    parser.add_argument(
        "--sheet_name", type=str, default="Table 1",
        help="Excel sheet name to read (default: 'Table 1')"
    )
    return parser.parse_args()


def filter_crassvirales(
    excel_path: Path, sheet_name: str = "Table 1"
) -> pd.DataFrame:
    try:
