...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--id_file", required=True, type=Path, help="Path to text file with sequence IDs (one per line).")
...
def extract_sequences(input_fasta: Path, target_ids: Set[str]) -> list:
    return [
        record for record in SeqIO.parse(str(input_fasta), "fasta")
